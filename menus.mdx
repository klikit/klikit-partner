...
<Warning>
  POS partners typically do **not** push menus. The klikit dashboard is the master catalogue for POS integrations; you read it via §1.1 and register Mapping (§1.3) against your own SKUs. Pushing a menu from a POS will overwrite the dashboard's structure — only do this if your contract explicitly designates you as the menu source of truth.
</Warning>
...
<Warning>
  Menu Push is **all-or-nothing for v1**. Any single validation error rejects the entire payload — partial menus are never persisted. Fix every reported error and resubmit the complete tree.
</Warning>
