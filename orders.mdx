...
- **Outbound webhooks** carry every order event you care about: `order.placed`, `order.updated`, `order.cancelled`, `order.amended`, plus downstream courier-lifecycle events `order.courier_assigned`, `order.picked_up`, `order.delivered` for delivery orders.
...
klikit will reject invalid transitions with `409 Conflict` and an `error.code` of `invalid_transition`.

---

## 2. Outbound Webhooks

All order lifecycle events for your credential's business scope are delivered to your registered `webhook_url` (see [Authentication §9](/authentication#9-onboarding-sequence)). Routing is by (business, brand, branch) scope — there is no per-event-type opt-in. Filter event types client-side if you only care about a subset. Every event is HMAC-signed.
...
| `event_type` | One of `order.placed`, `order.updated`, `order.cancelled`, `order.amended`, `order.courier_assigned`, `order.picked_up`, `order.delivered` |
...
| `order.courier_assigned` | Aggregator assigns a courier to a delivery order — read-only for partner | At-least-once |
| `order.picked_up` | Courier picks up the order from the store — read-only for partner | At-least-once |
| `order.delivered` | Order successfully delivered to customer — terminal for delivery flow, read-only for partner | At-least-once |

**Note:** `order.placed` is fired **once** when the order first becomes visible. Status transitions you drive yourself (accept, ready, etc.) are not separate webhooks — you are the source of those via the status callback API. If the **aggregator** drives a status change (customer cancels via the provider's app, courier picks up an order, etc.), the corresponding event fires.

<Tip>
  Courier-lifecycle events are read-only — you cannot drive them. They reflect the upstream aggregator's view of the delivery and let you complete the audit trail for ERP / invoicing flows. ERP partners typically **act only on** `order.delivered` (their invoicing trigger) and discard the others client-side — webhook routing delivers all events; filtering is your responsibility.
</Tip>
...
| 409 | `invalid_transition` | E.g. `accepted → placed` |
| 409 | `provider_rejected_cancel` | Aggregator refuses cancel (rider already assigned) |
| 400 | `invalid_reason_code` | `cancelled` without a recognised `reason_code` |
...
```http
GET /v1/partner/orders?status=delivered&from=2026-04-27T00:00:00Z&to=2026-04-27T23:59:59Z&limit=100&cursor=<opaque>
```
...
<Warning>
  The `(to - from)` window is capped at **90 days**. Wider ranges return `400 invalid_range`. For reconciliation jobs that need historical data beyond 90 days, page through multiple windows.
</Warning>
