...
<Warning>
  The `(placed_to - placed_from)` window is capped at **90 days**. Wider ranges return `400 invalid_range`. For reconciliation jobs that need historical data beyond 90 days, page through multiple windows.
</Warning>
