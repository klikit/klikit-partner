...
<Tip>
  Courier-lifecycle events are read-only — you cannot drive them. They reflect the upstream aggregator's view of the delivery and let you complete the audit trail for ERP / invoicing flows. ERP partners typically subscribe **only** to `order.delivered` (their invoicing trigger).
</Tip>
